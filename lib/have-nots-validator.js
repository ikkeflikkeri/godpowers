 ...
 * Have-Nots Validator
 *
 * Registry of mechanical checks against the 183 named have-nots from
 ...
 *   A-03 NFR not mapped              mechanical (PRD NFR vs ARCH map)
 *   A-14 critical flow undiagrammed  mechanical (section + ordered-step scan)
 *   A-15 capacity number w/o input   mechanical (section + source-signal scan)
 *   A-16 dependency w/o failure beh. mechanical (section + timeout/retry scan)
...
/** A-03: NFR not mapped to architectural choice */
...
// A-14, A-15, and A-16 name sections that did not exist in the ARCH anatomy
// before 5.14.0, so every ARCH written under an earlier version is missing all
// three. They report `warning`, not `error`: a lint error fails the arch gate
// (lib/gate.js treats errors as blocking), which would retroactively fail every
// in-flight project at a tier it already passed. The catalog still says "Fail."
// because the architect writing a NEW artifact must fill them; the mechanical
// layer grades an existing artifact as a gap to close, not a corruption. Do not
// promote these to `error` without a migration path for existing artifacts.
// This downgrade is REGISTERED in the "Severity Overrides" table of
// references/HAVE-NOTS.md (owner, rationale, sunset), and
// scripts/test-have-nots-tally.js asserts the applied severity matches that
// table, so the fork between the catalog and this validator stays declared.
const ARCH_SECTION_CHECKS = [
  {
    code: 'A-14',
    heading: /^##\s*(?:\d+\.\s*)?Critical Flows/im,
    label: 'Critical Flows',
    suggestion: 'Add an ordered interaction for every flow that crosses a trust boundary or touches more than two containers.'
  },
  {
    code: 'A-15',
    heading: /^##\s*(?:\d+\.\s*)?Capacity Envelope/im,
    label: 'Capacity Envelope',
    suggestion: 'Add the load inputs each target is measured under, with a source for every input.'
  },
  {
    code: 'A-16',
    heading: /^##\s*(?:\d+\.\s*)?Failure and Degradation/im,
    label: 'Failure and Degradation',
    suggestion: 'Add a timeout, a retry stance, and a user-visible degraded state for each dependency.'
  }
];

/** Locate one of the three post-5.14.0 ARCH sections, or report its absence. */
function archSection(content, spec) {
  const section = extractSection(content, spec.heading);
  if (section) return { section, finding: null };
  return {
    section: null,
    finding: {
      code: spec.code,
      ...
      line: 1,
      column: 1,
      message: `ARCH has no "${spec.label}" section.`,
...
function checkArchCriticalFlows(content) {
  const spec = ARCH_SECTION_CHECKS[0];
  const { section, finding } = archSection(content, spec);
  if (finding) return [finding];
  // An ordered interaction is what separates this section from a restatement
  // of the container diagram, so require at least one numbered step.
  const hasOrderedStep = /^\s*\d+[.)]\s+\S/m.test(section.body);
  if (hasOrderedStep) return [];
  return [{
    code: 'A-14',
    severity: 'warning',
    line: section.startLine,
    column: 1,
    message: 'Critical Flows section has no ordered steps.',
    suggestion: 'Number each step and name the caller, the callee, and where state is written.'
  }];
}

/** A-15: capacity number without an input */
function checkArchCapacityEnvelope(content) {
  const spec = ARCH_SECTION_CHECKS[1];
  const { section, finding } = archSection(content, spec);
  if (finding) return [finding];
  if (!/\d/.test(section.body)) {
    return [{
      ...
      line: section.startLine,
      column: 1,
      message: 'Capacity Envelope section contains no numbers.',
      suggestion: 'State the load inputs as numbers, then derive per-container load from them.'
    }];
  }
  // Numbers with nothing underneath them are the actual have-not. A source
  // column, a PRD reference, a measurement, or a labeled hypothesis all count.
  const hasSource = /\bsources?\b|\bmeasured\b|\bderived\b|\bPRD\b|\[HYPOTHESIS\]/i.test(section.body);
  if (hasSource) return [];
  return [{
    code: 'A-15',
    severity: 'warning',
    line: section.startLine,
    column: 1,
    message: 'Capacity Envelope names numbers but no source for them.',
    suggestion: 'Give every input a source, or label it [HYPOTHESIS] with the plan that validates it.'
  }];
}

/** A-16: dependency without failure behavior */
function checkArchFailureDegradation(content) {
  const spec = ARCH_SECTION_CHECKS[2];
  const { section, finding } = archSection(content, spec);
  if (finding) return [finding];
  const findings = [];
  // "A reasonable timeout" is the default timeout, which in most clients is no
  // timeout, so the number is the check.
  if (!/\b\d+(?:\.\d+)?\s*(?:ms|s|sec|secs|second|seconds|m|min|mins|minute|minutes)\b/i.test(section.body)) {
    findings.push({
      code: 'A-16',
      severity: 'warning',
      line: section.startLine,
      column: 1,
      message: 'Failure and Degradation section states no timeout with a number.',
      suggestion: 'Give each dependency a timeout in seconds, not an adjective.'
    });
  }
  // Only demand an idempotency stance where a retry is actually claimed.
  if (/\bretr(?:y|ies|ied|ying)\b/i.test(section.body) && !/\bidempoten/i.test(section.body)) {
    findings.push({
      code: 'A-16',
      severity: 'warning',
      line: section.startLine,
      column: 1,
      message: 'Failure and Degradation describes retries without an idempotency stance.',
      suggestion: 'For each retry, say whether the operation is idempotent and on what key.'
    });
  }
  return findings;
}
...
const ARTIFACT_CHECKS = {
  ...
  arch: [
    { code: 'A-03', fn: checkArchNfrMap },
    { code: 'A-14', fn: checkArchCriticalFlows },
    { code: 'A-15', fn: checkArchCapacityEnvelope },
    { code: 'A-16', fn: checkArchFailureDegradation }
  ],
  docmanifest: [
...
module.exports = {
  ...
  checkArchNfrMap,
  checkArchCriticalFlows,
  checkArchCapacityEnvelope,
  checkArchFailureDegradation,
