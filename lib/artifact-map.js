...
// Prompt surfaces: the runtime's own instruction files (skills, specialist
// prompts, references). Declared separately from TIER_ARTIFACTS on purpose:
// that map owns per-tier gate artifacts and its consumers (gate, dashboard)
// must never see prompt files. Cadence 'frozen': no mechanical loop may edit
// a prompt surface; the learning loop may only DRAFT a change as an
// improvement proposal (lib/improvement-proposals.js refuses any target
// outside this list), and only a human applies it via /god-review-changes.
const PROMPT_SURFACES = [
  { prefix: 'skills/', cadence: 'frozen' },
  { prefix: 'specialists/', cadence: 'frozen' },
  { prefix: 'references/', cadence: 'frozen' }
];

/**
 * Cadence tier for a prompt-surface path. Returns null for paths outside the
 * declared prompt surfaces. The path is normalized before the prefix check
 * so a traversal like `skills/../lib/x.js` cannot claim frozen cadence and
 * smuggle a non-prompt file past consumers that trust this answer.
 */
function promptCadence(relPath) {
  const path = require('path');
  const normalized = path.posix
    .normalize(String(relPath).replace(/\\/g, '/').replace(/^\.\//, ''));
  if (normalized.startsWith('..') || path.posix.isAbsolute(normalized)) return null;
  for (const surface of PROMPT_SURFACES) {
    if (normalized.startsWith(surface.prefix)) return surface.cadence;
  }
  return null;
}
