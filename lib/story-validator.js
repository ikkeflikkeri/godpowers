 ...
 *   validateStory(story, opts) -> findings   opts.mode: 'read' | 'authoring'
 *   validateForAuthoring(story) -> findings  section contract as errors
 ...
 *
 * `opts.mode` decides how hard the section contract bites:
 *
 *   'read' (default) - the section findings are warnings. This path also
 *     reads files written before a rule existed, and failing a story that was
 *     valid when it was written would make `/god-stories` unusable on any
 *     project with history.
 *   'authoring' - the section findings are errors. An agent writing a NEW
 *     unit has no legacy excuse: it is holding the template and the contract,
 *     so a missing section is a defect it is about to commit to disk.
 *
 * The two modes exist because the agent contract in
 * `specialists/god-storyteller.md` states these as hard failures while the
 * read path cannot. Before this option, those two surfaces disagreed and the
 * agent's stated have-nots were unenforceable.
 */
function validateStory(story, opts = {}) {
  const findings = [];
  const authoring = opts.mode === 'authoring';
  ...
  const contract = authoring ? 'error' : 'warning';
  if (isDecisionKind(story.kind)) {
    validateDecisionUnit(story, findings, contract);
  } else {
    validateSlice(story, findings, contract);
  }
...
/**
 * Validate a unit an agent is about to write. Section-contract findings are
 * errors here, so `specialists/god-storyteller.md` and
 * `specialists/god-cartographer.md` can state them as have-nots and have that
 * claim actually hold.
 */
function validateForAuthoring(story) {
  return validateStory(story, { mode: 'authoring' });
}

function validateSlice(story, findings, contract) {
...
function validateDecisionUnit(story, findings, contract) {
  ...
  // Resolution is not an authoring-time concern: a unit is written open and
  // answered later, so this stays a warning in both modes.
